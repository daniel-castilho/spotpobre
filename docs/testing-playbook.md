...
| **Likes**                       | `PutLikeServiceTest`, `DeleteLikeServiceTest`, `LikeStrategyFactoryTest`, `SongLikeStrategyTest`, `ArtistLikeStrategyTest`, `PlaylistLikeStrategyTest`       | Strategy dispatch, entity-existence checks, desired-state put/delete convergence (no version bump on satisfied state) through mocked port     |
