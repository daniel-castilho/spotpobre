...
| - | ---------------- | ---------------- | ----- |
| 1 | Codebase         | ✅ One repo, one app | Git repo `spotpobre-api`, `main` branch. No per-environment branches. Tagged releases `v0.1.0` … `v0.14.0` (`0.0.1-SNAPSHOT` between tags). |
| 2 | Dependencies     | ✅ Declared & pinned | `pom.xml` with `dependencyManagement` (spring-cloud-aws / testcontainers / rest-assured BOMs) and the Maven wrapper pinned to Maven 3.9.16 (`.mvn/wrapper/maven-wrapper.properties`). Reproducible build: `./mvnw clean package`. No lockfile equivalent in Maven — keep versions in `<properties>`. |
...
| 5 | Build, release, run | ✅ CI-gated       | Build = `./mvnw clean package` → executable Spring Boot jar; run = `java -jar ...` or `./mvnw spring-boot:run`. **CI green on GitHub** (`.github/workflows/ci.yml`): unit tests, boundary check, SpotBugs, the `*IT` E2E suite (Docker/Testcontainers), production build, Trivy image gate and runtime smoke on every push/PR. DynamoDB tables/bucket are created by the idempotent `scripts/seed-localstack.sh` (dev) — no versioned schema migration tooling yet. |
