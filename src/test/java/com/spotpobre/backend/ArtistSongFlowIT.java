...
class ArtistSongFlowIT extends AbstractIntegrationTest {
    ...
    void shouldCreateArtistAndUploadSongSuccessfully() {
        ...
        // 3. As ARTIST, upload a song for that album
        String songTitle = "Testcontainers Rock";
        String songId = given()
                .header("Authorization", "Bearer " + artistToken)
                .multiPart("request", String.format("{\"title\":\"%s\",\"albumId\":\"%s\"}", songTitle, albumId), "application/json")
        ...
        // 4. As any authenticated user, verify the song can be retrieved
        given()
                .header("Authorization", "Bearer " + artistToken)
                .when()
